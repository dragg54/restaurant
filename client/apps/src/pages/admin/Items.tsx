...
import Wrapper from '../../components/admin/layouts/wrapper/index'
import ItemCard from '../../components/admin/cards/ItemCard'
...
const Items = () => {
  ...
  const { isLoading, itemState, setIsLoading } = useContext(ItemContext)
      ...
      <div style={{ display: "flex", width: " 100%", flexWrap: "wrap", gap: "30px 15px", margin: "60px auto", boxSizing: "content-box" }}>
        {
          !isLoading && itemState && itemState.length > 0 && itemState.map((item) => {
            return (
              <ItemCard key={item._id} id={item._id!} price={+item.price!} description={item.description!} image={item?.image?.data!} name={item.name!} discount={item.discount!} rating={4.25} quantity={item.quantity}/>
            )
          })
        }
      </div>
